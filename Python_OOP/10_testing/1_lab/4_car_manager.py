...
class CarTests(TestCase):
    ...
    def test_model_empty_or_none_raises(self):
        ...
        with self.assertRaises(Exception) as ex:
            car = Car('test', None, 5, 40)
        self.assertEqual("Model cannot be null or empty!", str(ex.exception))

    def test_fuel_consumption_less_or_zero_raises(self):
        with self.assertRaises(Exception) as ex:
            car = Car('test', 'test', -1, 40)
        self.assertEqual("Fuel consumption cannot be zero or negative!", str(ex.exception))

        with self.assertRaises(Exception) as ex:
            car = Car('test', 'test', 0, 40)
        self.assertEqual("Fuel consumption cannot be zero or negative!", str(ex.exception))

    def test_fuel_capacity_less_or_zero_raises(self):
        with self.assertRaises(Exception) as ex:
            car = Car('test', 'test', 5, -1)
    ...
    def test_init(self):
        car = Car('test', 'test_model', 5, 40)

        self.assertEqual('test', car.make)
        self.assertEqual('test_model', car.model)
    ...
    def test_drive_not_enough_fuel_raises(self):
        ...
        self.assertEqual(00, car.fuel_amount)
        car.fuel_amount = 10
